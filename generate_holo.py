...
ASSET_DIR = Path(r"D:\project\Ouroboros-v2\app\src\Ouroboros.CLI\Assets\Avatar\Iaret")
...
def main():
    ...
    # Also copy to Ouroboros-v2's own asset dir if different
    v2_asset_dir = Path(r"D:\project\Ouroboros-v2\app\src\Ouroboros.CLI\Assets\Avatar\Iaret")
    if v2_asset_dir != ASSET_DIR:
        print(f"\nCopying to {v2_asset_dir}...")
        for _, dst_name in MAPPINGS.items():
            src = ASSET_DIR / dst_name
            dst = v2_asset_dir / dst_name
            if src.exists():
                import shutil
                shutil.copy2(src, dst)
                print(f"  ✓ Copied {dst_name}")
    
    print("\nDone! 7 holographic wireframe overlays generated.")
