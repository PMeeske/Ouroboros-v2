...
│   │   ├── idle.png                    ← Portrait bust (primary)
...
### 4.2 Iaret Front-Facing Asset

The Iaret front-facing asset (`fullbody_front.png`) is a PNG stored in the `app` submodule which is not initialized in this environment. As part of extraction:
- The original PNGs will be copied to `iaret/assets/avatar/`
- A placeholder SVG silhouette (`iaret/assets/banner/iaret-banner.svg`) will be created showing Iaret's front-facing form
- The `generate_holo.py` tool (moved to `iaret/tools/`) can regenerate holographic overlays
